...
4. **程序实现**
   ...
   + ==初始值==
     ...
     有一个方法，你先写代码，不管初始值；写完之后，检查完备性，看看有没有哪种情况没有覆盖到，这就作为初始值
...
我们看到磨损值的两部分并不是简单的关系，怎么分，都会导致一部分少，一部分多；这种情况下就可以考虑dp，从高层次去考虑问题，而不是仅仅从“如何分”来考虑。
...
3. 程序实现
   ...
   + 初始值必须是转移过程中你没有涵盖到的部分！比如这个！！这种情况（如果j从1开始）就是代码中没有包含到的部分！！
